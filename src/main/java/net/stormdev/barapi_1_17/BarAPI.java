package net.stormdev.barapi_1_17;
...
import net.stormdev.barapi_1_17.nms.FakeDragon;
import net.stormdev.barapi_1_17.nms.v1_9;
...
public class BarAPI implements Listener {
  ...
  private static Location getDragonLocation(Location loc) {
    ...
    if (pitch >= 55) {
      ...
      loc = loc.getBlock().getRelative(getDirection(loc), MarioKart.plugin.getServer().getViewDistance() * 16).getLocation();
    }
  ...
  public static void onEnable() {
    instance = new BarAPI();
    MarioKart.plugin.getServer().getPluginManager().registerEvents(instance, MarioKart.plugin);
    MarioKart.plugin.getLogger().info("Loaded");
  }
